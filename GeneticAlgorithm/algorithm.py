...
def setting_for_selection(pop_size, elitism=5, by_rank=8, by_fitness=8, by_tournament=13):
    global _elitism, _by_rank, _by_fitness, _by_tournament
    global _individuals_count
    _individuals_count = pop_size
    _elitism = _gere_selection_number(elitism, pop_size)
    _by_rank = _gere_selection_number(by_rank, pop_size)
    _by_fitness = _gere_selection_number(by_fitness, pop_size)
    _by_tournament = _gere_selection_number(by_tournament, pop_size)


def _gere_selection_number(number, pop_size):
    if number < 1:
        return int(number*pop_size)
    else:
        return number
...
class genetic_algorithm:
    def __init__(self):
        ...
        self.mean_mutation = total*_mutation_modify + total*_mutation_crossover + total*_mutation_inverse
        print("          - mean mutations by generation: %s" % self.mean_mutation)
        print("------------------------------------------\n\n")
        # create the pop
    ...
    def crossover_mutation(self, probability, population_to_mutate):
        real_proba = probability/self._len_genome
        count = len(population_to_mutate)
        for i in range(count):
            random_ind = self._individuals_class(self._individuals_class_param, None)  # new random individual
            individu = np.random.choice(population_to_mutate)
    ...
    def inverse_mutation(self, probability, population_to_mutate):
        count = len(population_to_mutate)
        for i in range(count):
            # test la proba
    ...
    def modify_mutation(self, probability, modify_range, number_type, population_to_mutate):
        real_proba = probability/self._len_genome
        number_individuals = len(population_to_mutate)
    ...
    def _mutate(self, population_to_mutate):
        self.crossover_mutation(_mutation_crossover, population_to_mutate)
        self.inverse_mutation(_mutation_inverse,population_to_mutate)
        self.modify_mutation(_mutation_modify, _modify_range, _modify_number_type, population_to_mutate)

    def crossover(self, number_of_crossing_point=-1):
        count = (self._mean_individuals_count - len(self.population) - self.mean_mutation) / 2
    ...
    def create_next_generation(self):
        ...
        # direct selection:
        double = False
        self.elitism(_elitism, allow_double=double)
        self.selection_by_rank(_by_rank, allow_double=double)
        self.selection_by_fitness(_by_fitness, allow_double=double)
        self.selection_by_tournament(_by_tournament, allow_double=double)

        # crossover
        self.crossover()
        self._mutate(self.population)
    ...
    def elitism(self, number_individuals, allow_double=False):
        for i in range(number_individuals):
            self.population = np.append(self.population, self.old_population[i])
            # supprime les individus selectionné:
        if allow_double:
            self.old_population = self.old_population[number_individuals:]

    def selection_by_fitness(self, number_individuals, allow_double=False):
        # creation du tableau de fitness
        fitness_arr = np.array(([]))
        for ind in self.old_population:
            fitness_arr = np.append(fitness_arr, [ind.fitness])
        if np.min(fitness_arr) < 0:
            fitness_arr += abs(np.min(fitness_arr))
        # proba
        proba = fitness_arr / np.mean(fitness_arr)
        proba = proba / np.sum(proba)
        # tire au sort X individus
        winners = np.random.choice(np.array(self.old_population), number_individuals, replace=allow_double, p=proba)
        self.population = np.append(self.population, winners)
        if not allow_double:
            for winner in winners:
                self.old_population.remove(winner)
        # self.old_population = np.delete(self.old_population, np.where(self.old_population == winners))

    def selection_by_rank(self, number_individuals, allow_double=False):
    ...
    def selection_by_tournament(self, number_individuals, tournament_participant=3, allow_double=False):
        # take x random individual and append the winner to the next generation
        for i in range(number_individuals):
            individus_selected = []
            # choie des x individus
            individus_selected = np.random.choice(self.old_population, tournament_participant, replace=allow_double)
            # tournois:
            winner = max(individus_selected, key=lambda x: x.fitness)
            # on ajoute le gagnant à la prochaine generation
            self.population = np.append(self.population, winner)
            if not allow_double:
                self.old_population.remove(winner)
            #self.old_population = np.delete(self.old_population, np.where(self.old_population == winner))
